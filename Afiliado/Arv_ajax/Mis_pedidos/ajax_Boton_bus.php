...
if(!empty($_POST)){
    ...
    { 
        try {
            ...
            $result_usuario =$sentenciaSQL->fetch(PDO::FETCH_LAZY);

            

            if ($result_usuario !== false) {
                $Tienda = $result_usuario['NameTienda'];
            }
             ...
             tbpedido.estadopago as estadopago_pedido,
               tbpedido.afiliado as tienda,
                ...
                tbpedido.total as TotalMonto,
                 ...
                 WHERE tbpedido.ref LIKE :p_id and 
                 tbpedido.afiliado= :p_dato1
                 GROUP BY tbpedido.ref 
                 ORDER BY fecha_pedido desc
                     " );
            $id1 = $id1.'%';
            $sentenciaSQL->bindParam(':p_id', $id1);
            $sentenciaSQL->bindParam(':p_dato1', $Tienda);
            $sentenciaSQL->execute();


            }else if($condicion =='Cliente' ) {
                ...
                tbpedido.total as TotalMonto,
                 ...
                 INNER JOIN tbpedidocliente 
                 on tbpedidocliente.ref = tbpedido.ref 
                 WHERE tbusuarios.usuario LIKE :p_id and
                 tbpedido.afiliado= :p_dato1
                 GROUP BY tbpedido.ref 
                 ORDER BY fecha_pedido desc " );
                $id1 = $id1.'%';
                ...
                tbpedido.total as TotalMonto,
                 ...
                 INNER JOIN tbpedidocliente 
                 on tbpedidocliente.ref = tbpedido.ref 
                     WHERE tbpedido.fecha >=  :p_id and tbpedido.fecha <= :p_id2 and
                 tbpedido.afiliado= :p_dato1
                      GROUP BY tbpedido.ref 
                      ORDER BY fecha_pedido desc " );
